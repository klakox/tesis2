...
<img src="logos2.png" style="width:65%">
...
Para aportar en el desarrollo de métodologías de imputación a partir de la normal multivariada en bases de datos de contaminación ambiental.
...
<p class="text">
Aplican un algoritmo de imputación múltiple para series de tiempo multivariadas en 10 estaciones de monitoreo de PM10 en Sao Pablo. Mediante el algoritmo EM estiman $\mu\,\,,\Sigma$ y ajustan modelos ARIMA a cada serie de tiempo univariada para actualizar las estimaciones. La imputación usando el algorimo EM regular arrojo estimaciones precisas bajo diferentes porcentajes de de faltas (5% y 40%) incluso con el incumplimiento del supuesto missing at random (MAR).
</div>
</div>
</div>
...
$$f(\mathbf{x})=\frac{1}{(2\pi)^{p/2}\left| \Sigma \right| ^{1/2}}e^{-\frac{1}{2}(\mathbf{x}-\boldsymbol{\mu})^T\Sigma^{-1}(\mathbf{x}-\boldsymbol{\mu})}\;\;\;\;\begin{matrix}
\mathbf{x}\in \mathbb{R}^p \\ 
\boldsymbol{\mu} \in \mathbb{R}^p\\ 
\Sigma \in \mathbb{R}^{p*p}
\end{matrix}$$
...
$$\boldsymbol{\hat{\mu}}=\mathbf{\bar{X}}=\frac{1}{n}\sum_{k=1}^{n}\mathbf{X_k }    \, \, \, \, \, Y \, \, \, \, \,
\hat{\Sigma}=\mathbf{S}=\frac{1}{n-1}\sum_{k=1}^{n}\left ( \mathbf{X_k}-\mathbf{\bar{X}} \right )\left (\mathbf{ X_k}-\mathbf{\bar{X}} \right )^{T}$$
...
$$\frac{n}{6}\hat{\gamma_{1}} =\frac{n}{6} \frac{1}{n^2} \sum_{b=1}^n \sum_{c=1}^n d_{bc}^3 \sim \chi_{gl = \frac{p(p-1)(p+1)}{6}}$$
...
$$d_{bc}=(x_b-\bar{x})^TS^{-1}(x_c-\bar{x})$$
...
htmltools::div(pajpl, align="center" )  # Result is now an HTML object
